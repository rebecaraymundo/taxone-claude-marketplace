# MTZ-Report_Fiscal-Diferencial-de-Aliquota-Por-Docto-Resumido

- **Fonte:** MTZ-Report_Fiscal-Diferencial-de-Aliquota-Por-Docto-Resumido.docx
...
__                   Relatório de Conferência do Diferencial de Alíquota por Documento \(resumido\)__

__Localização__: Menu Básicos, Módulo Report Fiscal, item Obrigações Acessórias 🡪 ICMS 🡪 Diferencial de Alíquota p/ DataMart  🡪 Documento \(resumido\)
...
Desenvolvimento de um novo relatório de conferência do diferencial de alíquota\.
...
__Geração por inscrição estadual única – __Este campo é um checkbox através do qual o usuário poderá solicitar a geração do relatório de forma centralizada por inscrição estadual única\. Seu default na abertura da tela é = desmarcado\.
...
__Origem dos dados__: Tabelas dos Documentos Fiscais – Capa e Itens de Mercadoria  \(SAFX07 e SAFX08\)

Critérios de recuperação dos documentos na SAFX07 e SAFX08:

     \- __COD\_EMPRESA__ – Código da empresa do  login

     \- __COD\_ESTAB__ – Estabelecimentos selecionados em tela 

     \- __COD\_CLASS\_DOCFIS __– Deve ser = 1 ou 3 

     \- __DATA\_FISCAL__ – Data fiscal que se enquadre no período informado em tela 

     \- __MOVTO\_E\_S__ – Apenas notas de entrada \(MOVTO\_E\_S <> 9\)

     \- __SITUACAO__ – Somente os documentos *não* cancelados	

     \- __Verificação da UF__ 🡪 Para verificação da UF será utilizado o parâmetro 67 da “Parametrização por UF” do Módulo ICMS\. 

                                         Serão recuperadas *apenas* as notas em que e UF da pessoa fis/jur __ou__ a UF Origem da nota, dependendo
...
__                                     Senão__
     ...
     \- __Diferença de Alíquota__ \(DIF\_ALIQ\_TRIB\_ICMS\) – Este campo informa o percentual da diferença de alíquota entre as duas UF’s 
       ...
       A verificação deste campo depende se a nota tiver itens ou não, da seguinte forma:
                    ...
                    Notas com itens 🡪 Neste caso, serão considerados *apenas* os itens com o campo “44\-Diferença Alíquota ICMS”
                                                   ...
                                                   \(DIF\_ALIQ\_TRIB\_ICMS\) preenchido com um valor > 0;

                    Notas sem itens 🡪 Neste caso, a nota será considerada *apenas* se o campo “36\-Diferença Alíquota ICMS”

                                                   \(DIF\_ALIQ\_TRIB\_ICMS\) estiver preenchido com um valor > 0;
...
__Sobre a geração por inscrição estadual única__:
...
__Cálculo do Diferencial de Alíquota: \(Padrão\)__

Este relatório gera uma linha de detalhe *para cada documento fiscal* \(ver layout na __RN04__\)\.
...
*\(observar que este relatório não tem detalhamento de itens, o valor do diferencial é demonstrado por nota\) *
         ...
         \- Para as notas *com* itens 🡪 o valor será calculado __*item a item*__, e o total da nota será a totalização dos valores calculados para
...
A opção 4 indica que o valor não será calculado, pois ele já é informado na interface da nota \(campo “Vlr\. Dif\. Alíquota” da tela de manutenção das notas, que corresponde à coluna VLR\_OUTROS1\)\.

__Parâmetro do Ferramentas__

__Forma de Cálculo do Difal __

1 \- \(Base Tributada\)

\[ Base Tributada \* DIF\_ALIQ\_TRIB\_ICMS / 100 \]	

2 \- \(Base Tributada \+ Base Outras\)

\[ \(Base Tributada \+ Base Outras\) \* DIF\_ALIQ\_TRIB\_ICMS / 100 \]

3 \- \(Valor Contábil – Valor ST – Base Redução – 

       Base Isenta\) 

\[ \(Valor Contábil do Item – \(Base Isenta \+ Base Redução\) – Valor do ICMS\-ST\) 

                                                  \* DIF\_ALIQ\_TRIB\_ICMS / 100 \]

4 \- \(Valor Informado\)

Nesse caso o cálculo não será realizado, e ao invés disso, o valor será recuperado do campo “Vlr\. Dif\. Alíquota” do item \(ou nota, no caso das notas sem itens\)\.

\(trata\-se da coluna VLR\_OUTROS1\)
...
__*\(\*\) *__*Módulo ferramentas, menu “Parâmetros do Sistema 🡪 Parâmetros por Estabelecimento”\. \(*__*Obs\.: *__*Trata\-se da mesma Regra do tópico acima “Cálculo do Diferencial de Alíquota: \(Padrão\)”*
...
<a id="OLE_LINK14"></a><a id="OLE_LINK15"></a><a id="OLE_LINK16"></a><a id="OLE_LINK17"></a>     Se UF do Estabelecimento = “__RS__” 🡪 <a id="OLE_LINK42"></a><a id="OLE_LINK43"></a><a id="OLE_LINK44"></a><a id="OLE_LINK45"></a>Neste caso será utilizada a regra de cálculo descrita na __RN10\.a__;
     ...
     Se UF do Estabelecimento = “__SP__”  Neste caso será utilizada a regra de cálculo descrita na __RN010\.d__;

     Se UF do Estabelecimento = “__ES”, “BA” ou “SC”__  Neste caso será utilizada a regra de cálculo descrita na __RN010\.e__;
...
__Regra do cálculo para Diferencial de Alíquota– \(UF = RO\)\.__
...
Valor do Difal: \[ \(\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-  \*  Alíq\. Destino\) \-  \(\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-  \* Alíq\. Interestadual\) \]
...
__                                   Diferencial de Alíquota por Documento – Período: 99/99/9999 a 99/99/9999__
...
__   Número Documento__

__Série/__

__Subsérie__

__         Chave de Acesso NFe__

__      Valor Diferença __
...
Este relatório gera uma linha de detalhe *para cada documento fiscal* \(ver regras do cálculo do dif\. alíquota na __RN03__\)\.
...
__Ordenação dos dados__ 🡪 Data Fiscal, Número do Documento, Série e Subsérie;
...
Observar que no caso de geração por I\.E\.U\., os dados apresentados serão do estabelecimento centralizador, em nome do qual será gerado o relatório\. 

Terceira linha do cabeçalho

Nesta linha será mostrado o título do relatório e o período solicitado em tela \(data inicial e data final\), conforme o layout\. 
...
__Número Documento__

Número do documento fiscal \(SAFX07\)

__Série/Subsérie__

Série e subsérie do documento fiscal \(SAFX07\)

__Chave de Aceso NFe__

Chave de acesso da nota fiscal eletrônica \(SAFX07, campo 226\)

__Valor Diferença de Alíquota__

Valor do diferencial de alíquota calculado para o documento, conforme descrito na __RN03__

__Linha de Totalização__:

__Total do Estabelecimento__

Ao finalizar as notas de um determinado estabelecimento, será gerada uma linha com a totalização da coluna “Valor Diferença de Alíquota”\. 
