...
\- Módulo ICMS > menu “Apuração > Apuração do ICMS > Lançamentos Complementares > Apuração ICMS > Ajuste SINIEF > __Aba Lançamentos de Valores__”, campos 🡪 “Data Apuração, Operação Apuração, Descrição, Classe Lançamento, Amparo/Texto/Ocorrencia, Subitem Amp\./Texto/Ocor\. e Código de Ajuste \(Sped Fiscal – Reg E11/E220\)\.”

\- Tabela Discriminação Item de Apuração \(ITEM\_APURAC\_DISCR\) para o Tipo de Livro = ‘108’ \- Apuração Normal \(COD\_TIPO\_LIVRO = ‘108’\)
...
Serão recuperados os dados da parametrização \(ITEM\_APURAC\_DISCR\) do estabelecimento: DAT\_APURACAO, COD\_OPER\_APUR, DSC\_ITEM\_APURACAO, COD\_CLASSE, COD\_AMPARO\_LEGAL, COD\_SUB\_ITEM\_OCORR, COD\_AJUSTE\_ICMS considerando os seguintes critérios:
...
__Passo 2__: Gravar na tabela de Item de Apuração \(ITEM\_APURAC\_DISCR\) e efetuar um novo lançamento a cada registro de lançamento selecionado em tela, com os seguintes dados:
