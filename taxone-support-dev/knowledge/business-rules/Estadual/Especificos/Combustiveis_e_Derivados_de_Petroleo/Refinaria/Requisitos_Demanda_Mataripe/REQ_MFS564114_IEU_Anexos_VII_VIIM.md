# REQ_MFS564114_IEU_Anexos_VII_VIIM

- **Fonte:** REQ_MFS564114_IEU_Anexos_VII_VIIM.docx
- **Modificado:** 2023-09-14
- **Tamanho:** 114 KB

---

THOMSON REUTERS

__MFS564114__

__Scanc__

__Scanc – Refinaria – Geração dos Anexos VII e VII\-M__

Revisões

__Data__

__OS/Chamado__

__Descrição__

__Autor__

11/09/2023

MFS 564114

Inclusão da opção IEU e Filtro de UF na parametrização da Rotina de Geração dos Anexos VII e VII\-M – Refinaria \- Scanc

Graciela Soares

  

Sumário

[1\.	INTRODUÇÃO	3](#_Toc145439872)

[1\.1	Documentos e Base Legal para a Solução	3](#_Toc145439873)

[2\.	SOLUÇÃO FUNCIONAL / ESCOPO	3](#_Toc145439874)

[2\.1	Procedimentos para Fábrica	3](#_Toc145439875)

[2\.1\.1	Inclusão da opção de Filtro por Inscrição Estadual Única:	3](#_Toc145439876)

[2\.1\.2	Inclusão da opção de Filtro por UF:	3](#_Toc145439877)

[2\.2	Procedimentos para Interface	4](#_Toc145439878)

[3\.	PROCEDIMENTOS PARA CONTEÚDO	4](#_Toc145439879)

[3\.1	Criação / Alteração – Tabelas \(Manual de Layout\)	4](#_Toc145439880)

[3\.1\.1	Manual de Layout	4](#_Toc145439881)

[3\.2	Criação / Alteração em telas e relatórios \(manual operacional, roteiro e help\)	4](#_Toc145439882)

[3\.2\.1	Geração dos Anexos VII e VII\-M	4](#_Toc145439883)

[3\.3	Criação / Alteração de Tabelas \(Fixas, Acessórias, Básicas\)	5](#_Toc145439884)

[3\.4	Criação / Alteração de Roteiro Operacional	5](#_Toc145439885)

[3\.5	Outras informações que necessitarão ser atualizadas \(help, manual operacional etc\)	5](#_Toc145439886)

[3\.6	Informações p/ Carta do Patch	5](#_Toc145439887)

[4\.	TESTES	6](#_Toc145439888)

# <a id="_Toc200962059"></a><a id="_Toc200996459"></a><a id="_Toc200996577"></a><a id="_Toc200998055"></a><a id="_Toc354574223"></a><a id="_Ref354574309"></a><a id="_Ref354574320"></a><a id="_Toc354574398"></a><a id="_Toc354574442"></a><a id="_Toc354574527"></a><a id="_Toc354578073"></a><a id="_Toc354578300"></a><a id="_Toc354578991"></a><a id="_Toc354579132"></a><a id="_Toc145439872"></a>INTRODUÇÃO

Ajuste no Filtro de Parametrização da Rotina de Geração dos anexos VII e VII\-M dos Movimentos de Refinaria do modulo de Combustíveis e Derivados de Petróleo:

\- Inclusão da opção de filtro por Estabelecimento com Inscrição Estadual Única

\- Inclusão de Filtro por UF

## <a id="_Toc145439873"></a>Documentos e Base Legal para a Solução

# <a id="Premissas"></a><a id="_Toc200962060"></a><a id="_Toc200996460"></a><a id="_Toc200996578"></a><a id="_Toc200998056"></a><a id="_Toc354574225"></a><a id="_Toc354574400"></a><a id="_Toc354574444"></a><a id="_Toc354574529"></a><a id="_Toc354578075"></a><a id="_Toc354578302"></a><a id="_Toc354578993"></a><a id="_Toc354579134"></a><a id="_Toc145439874"></a>SOLUÇÃO FUNCIONAL / ESCOPO

## <a id="_Toc200962061"></a><a id="_Toc200996461"></a><a id="_Toc200996579"></a><a id="_Toc200998057"></a><a id="_Toc354574226"></a><a id="_Toc354574401"></a><a id="_Toc354574445"></a><a id="_Toc354574530"></a><a id="_Toc354578076"></a><a id="_Toc354578303"></a><a id="_Toc354578994"></a><a id="_Toc354579135"></a><a id="_Toc145439875"></a>Procedimentos para Fábrica

### <a id="_Toc145439876"></a>Inclusão da opção de Filtro por Inscrição Estadual Única:

__Localização no DW__: Menu Específicos  Combustíveis e Derivados de Petróleo,

Menu \-> Movimento Refinaria \-> Geração dos Anexos VII e VII\-M 

Inclusão de um __*Check\-Box*__ com filtro de nome “Inscrição Estadual Única” que ao ser marcado filtre os estabelecimentos de centralizados por inscrição estadual única e também os descentralizados\.

Exemplo de Tela:

![](data:image/png;base64,)

### <a id="_Toc145439877"></a>Inclusão da opção de Filtro por UF:

__Localização no DW__: Menu Específicos  Combustíveis e Derivados de Petróleo,

Menu \-> Movimento Refinaria \-> Geração Anexo VII e VII\-M

Inclusão de uma __*Lista*__ com filtro de nome __“Pesquisa UF \(Estabelecimento\)”__ contendo todas as UF´s constantes na tabela __“Estado”__ mais a opção __“Todos”__\.

Na seleção do estado correspondente deve\-se verificar na tabela__ “Estabelecimento”__ a UF correspondente e apresentar na lista de seleção todos os Estabelecimentos do estado selecionado\. 

Exemplo de Tela:

![](data:image/png;base64,)

## <a id="_Toc354574228"></a><a id="_Toc354574403"></a><a id="_Toc354574447"></a><a id="_Toc354574532"></a><a id="_Toc354578078"></a><a id="_Toc354578305"></a><a id="_Toc354578996"></a><a id="_Toc354579137"></a><a id="_Toc145439878"></a>Procedimentos para Interface

<a id="_Toc200962071"></a><a id="_Toc200996470"></a><a id="_Toc200996588"></a><a id="_Toc200998068"></a>Não há

# <a id="_Toc354574229"></a><a id="_Toc354574404"></a><a id="_Toc354574448"></a><a id="_Toc354574533"></a><a id="_Toc354578079"></a><a id="_Toc354578306"></a><a id="_Toc354578997"></a><a id="_Toc354579138"></a><a id="_Toc145439879"></a>PROCEDIMENTOS PARA CONTEÚDO

<a id="_Toc199304378"></a><a id="_Toc200962072"></a><a id="_Toc200996471"></a><a id="_Toc200996589"></a><a id="_Toc200998069"></a>

## <a id="_Toc354574230"></a><a id="_Toc354574405"></a><a id="_Toc354574449"></a><a id="_Toc354574534"></a><a id="_Toc354578080"></a><a id="_Toc354578307"></a><a id="_Toc354578998"></a><a id="_Toc354579139"></a><a id="_Toc145439880"></a>Criação / Alteração – Tabelas \(Manual de Layout\)

### <a id="_Toc145439881"></a>Manual de Layout 

Não há

## <a id="_Toc200996472"></a><a id="_Toc200996590"></a><a id="_Toc200998070"></a><a id="_Toc354574231"></a><a id="_Toc354574406"></a><a id="_Toc354574450"></a><a id="_Toc354574535"></a><a id="_Toc354578081"></a><a id="_Toc354578308"></a><a id="_Toc354578999"></a><a id="_Toc354579140"></a><a id="_Toc145439882"></a>Criação / Alteração em telas e relatórios \(manual operacional, roteiro e help\)

<a id="OLE_LINK3"></a><a id="OLE_LINK4"></a><a id="OLE_LINK7"></a>

Alteração no Manual Operacional do módulo Combustíveis e Derivados de Petróleo\.

### <a id="_Toc145439883"></a>Geração dos Anexos VII e VII\-M 

__Localização no DW__: Menu Específicos  Combustíveis e Derivados de Petróleo,

Menu \-> Movimento Refinaria \-> Geração Anexo VII e VII\-M

Nesta rotina o usuário pode fazer a [geração](https://webhelp.thomsonreuters.com.br/Site/HELPDW/V2R010/Especificos/combustiveis_e_derivados/rot_safcomb.htm#proc_gera_anexoVI) do Anexo VII e VII\-M\. Estes anexos são gerados para todas as UF's em que a empresa/estabelecimento tenha Registro Estadual cadastrado mesmo que a UF não tenha informações a apresentar em nenhum dos quadros \(anexo SEM MOVIMENTO\)\.

Dados a serem informados:

__Mês/Ano Referência:__ Informar o mês/ano de referência, que deseja fazer a geração\.

__Excluir Registros Inseridos pela Manutenção:__ Caso o usuário deseje excluir os registros inseridos pela manutenção, deve habilitar este campo\.

__Pesquisa UF \(Estabelecimento\):__ lista contendo todas as UF’s mais a opção “Todos”\. 

__Inscrição Estadual Única:__ caso essa opção seja selecionada, serão apresentados apenas os estabelecimentos Centralizadores e Descentralizados\.

__Estabelecimentos:__ Lista todos os estabelecimentos cadastrados, para que o usuário selecione quais estabelecimentos que deseja fazer a geração\. Tendo a opção de selecionar todos e desmarcar todos automaticamente\.

## <a id="_Toc145439884"></a>Criação / Alteração de Tabelas \(Fixas, Acessórias, Básicas\)

<a id="OLE_LINK13"></a>

## <a id="_Toc354574233"></a><a id="_Toc354574408"></a><a id="_Toc354574452"></a><a id="_Toc354574537"></a><a id="_Toc354578083"></a><a id="_Toc354578310"></a><a id="_Toc354579001"></a><a id="_Toc354579142"></a><a id="_Toc145439885"></a>Criação / Alteração de Roteiro Operacional

## <a id="_Toc354574234"></a><a id="_Toc354574409"></a><a id="_Toc354574453"></a><a id="_Toc354574538"></a><a id="_Toc354578084"></a><a id="_Toc354578311"></a><a id="_Toc354579002"></a><a id="_Toc354579143"></a><a id="_Toc145439886"></a>Outras informações que necessitarão ser atualizadas \(help, manual operacional etc\)

## <a id="_Toc426992014"></a><a id="_Toc428981618"></a><a id="_Toc145439887"></a>Informações p/ Carta do Patch

<a id="OLE_LINK49"></a><a id="OLE_LINK16"></a><a id="OLE_LINK42"></a><a id="OLE_LINK46"></a>__Carta do Patch – DW e TAXONE__:

* *

__Módulo__: Combustíveis e Derivados de Petróleo

  


__Resumo da alteração: __ Inclusão de novas opções de filtros na tela de parametrização da Rotina de Geração dos anexos VII e VII\-M – Refinaria\.

  
Foram disponibilizadas opções adicionais de Filtro na tela de Parametrização da Rotina de Geração dos anexos VII e VII\-M dos Movimentos de Refinaria do módulo de Combustíveis e Derivados de Petróleo:

\- Inclusão da opção de filtro por Estabelecimento com Inscrição Estadual Única

\- Inclusão de Filtro por UF

 

Funcionalidades atualizadas:  
  
Módulo Impactado: Combustíveis e Derivados de Petróleo

Menu \-> Movimento Refinaria \-> Geração Anexo VII e VII\-M

# <a id="_Toc200962062"></a><a id="_Toc200996464"></a><a id="_Toc200996582"></a><a id="_Toc200998062"></a><a id="_Toc354574235"></a><a id="_Toc354574410"></a><a id="_Toc354574454"></a><a id="_Toc354574539"></a><a id="_Toc354578085"></a><a id="_Toc354578312"></a><a id="_Toc354579003"></a><a id="_Toc354579144"></a><a id="_Toc145439888"></a>TESTES

= = = = =

