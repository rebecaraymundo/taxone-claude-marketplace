# MTZ_Tela_Parametro_Geracao_Quadro_48_por_Tipo_de_AtividadexNatureza_de_Operacao

- **Fonte:** MTZ_Tela_Parametro_Geracao_Quadro_48_por_Tipo_de_AtividadexNatureza_de_Operacao.docx
...
Por Código do Tipo de Atividade x Natureza de Operação
...
[1\.	Regras dos Campos	3](#_Toc499637065)

# <a id="regras_campos"></a><a id="_Toc350763252"></a><a id="_Toc499637065"></a>Regras dos Campos 

__Localização da tela:__ 

Módulo: Estadual > DIME\-SC

		Menu: Parâmetros > Informações de ICMS > Quadro 47 e 48 > __Por Tipo de Atividade x Natureza de Operação__

	        __Título da tela: __Parâmetros p/Geração do Quadro 48 por Tipo de Atividade x Natureza de Operação\.
...
__Funcionamento da tela:__ Nesta tela serão parametrizados os dados necessários para geração do registro 48 – Informações para Rateio do Valor Adicionado\. Neste item o usuário poderá identificar os códigos do tipo de atividade, e CFOP's/ Naturezas de Operação que compõem as operações do quadro 48 da DIME\-SC, relacionado às notas fiscais de prestações de serviços e fornecimento de energia elétrica\.

__Observações 1:__ A recuperação dos dados para a geração do Quadro 48, é feita através do documentos fiscais com operação de prestação de serviços e fornecedores de energia elétrica\. Estes documentos serão identificados através dos CFOPs e Naturezas de Operação parametrizadas nesta opção\.

<a id="OLE_LINK1"></a><a id="OLE_LINK2"></a><a id="OLE_LINK3"></a><a id="OLE_LINK4"></a>__Observações 2: __A partir de janeiro de 2014, será necessária essa parametrização para que seja feito um DE\-PARA com a tela __“Parâmetros p/Geração dos Quadros 47 e 48 por CFOP x Natureza de Operação” __considerando o Código Tipo de Atividade selecionada\. Exemplo: O contribuinte já possui o CFOP 5251 com a Natureza de Operação 001 na tela __“Parâmetros p/Geração dos Quadros 47 e 48 por CFOP x Natureza de Operação” __para Operação de Energia Elétrica, ele precisa informar para esse mesmo CFOP/ Natureza de Operação o Código Tipo de Atividade na nova tela __“Parâmetros p/Geração do Quadro 48 por Tipo de Atividade x Natureza de Operação”__\.

__Observação 3: __Não poderá ser parametrizado para o mesmo estabelecimento código de CFOP/ Natureza de Operação iguais para código de atividade distinto\.
