# MTZ-ISISS_Vitoria-Geracao_do_Meio_Magnetico-sit_especial

- **Fonte:** MTZ-ISISS_Vitoria-Geracao_do_Meio_Magnetico-sit_especial.docx
...
DW \- MUNICIPAL \- VITORIA \- Atendimento a constução civil/telecomunicações/utilities

Este documento tem como objetivo criar a geração do meio magnético para os segmentos de Construção Civil, Utilities e Telecom para os municípios atendidos pelo validador SUBTI/PMV\. 
...
Esse campo deverá exibir somente os estabelecimentos que não pertencem ao município em questão do validador  Vitória e que estejam devidamente licenciados e cadastrados no menu: MasterSAF DW – Manutenção – Cadastros – Inscrições Municipais com uma ou mais inscrições municipais eventuais válidas para o município em questão\.
...
- Classificação do documento \(cód\_class\_doc\_fis = ‘2’, ‘3’\) ou Classificação do documento \(cod\_class\_doc\_fis = ‘9’\) e codigo do documento \(cod\_docto = ‘RPA’\)
...
- Se o campo DT\_PAGTO\_NF da tabela DWT\_DOCTO\_FISCAL \(campo 175 da SAFX07\) estiver preenchido e dentro do período de referência, o documento fiscal deve ser recuperado\.

Se o campo acima não estiver preenchido na tabela DWT\_DOCTO\_FISCAL, recuperar o documento fiscal através do campo DATA\_FISCAL da tabela DWT\_DOCTO\_FISCAL, a data fiscal deve estar dentro do período de referência\.

O arquivo deve conter apenas o registro de serviços tomados com a mesma estrutura do layout\. \(vide MTZ – ISISS Vitoria \- Geracao do Meio Magnético\.doc\)
