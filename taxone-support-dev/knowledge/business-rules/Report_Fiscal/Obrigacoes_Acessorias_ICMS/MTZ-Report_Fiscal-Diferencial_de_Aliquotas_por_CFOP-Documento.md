---
source: "MTZ-Report_Fiscal-Diferencial_de_Aliquotas_por_CFOP-Documento.doc"
category: Report_Fiscal
converted: auto
---

Report Fiscal - Relatório de Diferencial de Alíquota por CFOP/Documento
...
CH95372
Considerar Base Isenta ICMS para Dif. Aliquota
Considerar Base Isenta ICMS para o calculo do Diferencial de Aliquota.
CH7140_2012
Corrigir a geração do campo UF Origem
Corrigir a geração do campo UF Origem
OS3819
Alteração da apuração do Dif. Alíquota
Alteração da apuração do diferencial de alíquota para considerar o parâmetro 67 (Parametrização por UF do Módulo ICMS). (ver RN07)
MFS65000
Inclusão do campo FECEP Diferencial de Alíquota
RN08: Inclusão do campo FECEP Diferencial de Alíquota
MFS76479
...
MFS859953
Alteração para calcular o DIFAL
Ajuste do Cálculo do DIFAL com Base Dupla p/ São Paulo em atendimento às operações de contribuintes do Simples Nacional - Base Dupla - RN09.d
MFS938348
Alteração para calcular o DIFAL
Ajuste do Cálculo do DIFAL com Base Dupla p/ São Paulo em atendimento às operações de contribuintes do Simples Nacional - Base Dupla - RN09.d
...
Este campo deve vir por default marcado (vide RN10).
...
CH95372
RN06
Na geração do relatório, o campo UF Origem deve recuperar a informação do campo 117 - UF_ORIG_DEST (SAFX07), quando:

- Campo 03 - MOVTO_E_S = 'E' ou 'S';
- Campo 11 - DATA_EMISSAO for pertinente ao período de emissão do relatório.

Esta regra deve contemplar a geração por IEU.
...
RN08
Após a coluna Valor Diferença de Alíquota, incluir a coluna "FECEP Dif. Alíq.", demonstrando o conteúdo do campo 139 da SAFX08 (VLR_FECP_DIFALIQ).
MFS65000
RN09
...
Senão
  Se (parâmetro 92 Marcado):
     ...
     Se UF do Estabelecimento = "RS" --> Neste caso será utilizada a regra de cálculo descrita na RN09.a;
     Se UF do Estabelecimento = "GO" --> Neste caso será utilizada a regra de cálculo descrita na RN09.b;
     Se UF do Estabelecimento = "PR" ou "MG" --> Neste caso será utilizada a regra de cálculo descrita na RN09.c;
     Se UF do Estabelecimento = "SP" --> Neste caso será utilizada a regra de cálculo descrita na RN09.d;
     Se UF do Estabelecimento = "ES", "BA" ou "SC" --> Neste caso será utilizada a regra de cálculo descrita na RN09.e;
     Se UF do Estabelecimento = "PA" --> Neste caso será utilizada a regra de cálculo descrita na RN09.f;
     Se UF do Estabelecimento = "MA" --> Neste caso será utilizada a regra de cálculo descrita na RN09.g;
     Se UF do Estabelecimento = "RO" --> Neste caso será utilizada a regra de cálculo descrita na RN09.h;
     Se UF do Estabelecimento = "AC" --> Neste caso será utilizada a regra de cálculo descrita na RN09.i;
     Se UF do Estabelecimento = "RJ" --> Neste caso será utilizada a regra de cálculo descrita na RN09.j; 
     Se UF do Estabelecimento = "SE" --> Neste caso será utilizada a regra de cálculo descrita na RN09.k;
     Se UF do Estabelecimento = "RR" --> Neste caso será utilizada a regra de cálculo descrita na RN09.l;
     Se UF do Estabelecimento = "TO" --> Neste caso será utilizada a regra de cálculo descrita na RN09.m;
     Se UF do Estabelecimento = "AM" --> Neste caso será utilizada a regra de cálculo descrita na RN09.n; 
     Se UF do Estabelecimento = "AP" --> Neste caso será utilizada a regra de cálculo descrita na RN09.o;
     Se UF do Estabelecimento = "PE" --> Neste caso será utilizada a regra de cálculo descrita na RN09.p
     Se UF do Estabelecimento = "MS" --> Neste caso será utilizada a regra de cálculo descrita na RN09.q;
     Se UF do Estabelecimento = "PI" --> Neste caso será utilizada a regra de cálculo descrita na RN09.r;
     Se UF do Estabelecimento = "AL" --> Neste caso será utilizada a regra de cálculo descrita na RN09.s;
     Se UF do Estabelecimento = "PB" --> Neste caso será utilizada a regra de cálculo descrita na RN09.t;
     Se UF do Estabelecimento = "RN" --> Neste caso será utilizada a regra de cálculo descrita na RN09.u;
