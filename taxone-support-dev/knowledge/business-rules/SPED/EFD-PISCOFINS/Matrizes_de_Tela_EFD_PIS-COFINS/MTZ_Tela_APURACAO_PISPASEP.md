# MTZ_Tela APURACAO_PISPASEP

- **Fonte:** MTZ_Tela APURACAO_PISPASEP.docx
...
# \(EFD\-PIS/PASEP – COFINS\) – Apuração – PIS/PASEP
...
Alteração na tela Apuração – PIS/PASEP

Incluir na tela de Apuração \(PIS/PASEP\) os campos Valor Limite da Contribuição Cumulativa e Não Cumulativa a Descontar e Criar a  Pasta de Retenção na Fonte\.

14/12/2011

OS4584

Upload do registro M205 

Inclusão na tela a funcionalidade upload para importação do XML compondo o registro M205\.
...
Inclusão dos campos para a visualização dos limites \(na tela principal, da apuração\) estes campos estão nas telas de Retenções a Utilizar na Apuração
...
Na Pasta do Crédito de PIS/PASEP no Período – M100 → \(aba “Crédito de PIS/PASEP no Período – M100”\)

Se o parâmetro ‘’Informar os Descontos/Deduções manualmente, nas telas de Apuração PIS/PASEP e COFINS’’ estiver marcado na tela dados iniciais, na tela de manutenção do registro M100 o campo ‘’Valor do Crédito Disponível, descontado da contribuição apurada no próprio período’’ deve ficar habilitado para a manutenção \(alteração\)\. 
...
__Localização:__ Sped → EFD – Escrituração Fiscal Digital \- PIS/PASEP\-COFINS \- __Menu:__ Obrigações → Manutenção → Apuração PIS/PASEP → Pasta Retenção na Fonte PIS/PASEP Apurada no Período\.

OS3169\-GE28

__RN04__

A tela terá os seguintes campos e definições:
...
Tela nos mesmos moldes da manutenção do M100, M110\.\.\. 
...
Atualizar os valores da tela de “Crédito de PIS/PASEP no Período – M100” \(Localização: Sped → EFD – Escrituração Fiscal Digital \- PIS/PASEP\-COFINS \- Menu: Obrigações → Manutenção → Apuração PIS/PASEP → Pasta Crédito de PIS/PASEP no Período – M100\), considerando as alterações realizadas na aba “Detalhamento da Base de Cálculo do Crédito – M105”, conforme definições descritas na RN11\.

No momento que o usuário clicar no botão “Confirmar” da aba “Detalhamento da Base de Cálculo do Crédito – M105” e tendo alterado valores na referida tela, estas alterações irão refletir nos valores apurados para a tela “Crédito de PIS/PASEP no Período – M100” que é pai dos registros M105\. Este reflexo se dá no conteúdo dos campos:

- Campo “Valor da Base de Cálculo do Crédito”: este campo deve ser atualizado com o resultado da somatória do valor do campo “Valor da Base de Cálculo do Crédito, vinculada ao tipo de crédito escriturado” de todos os registros da aba “Detalhamento da Base de Cálculo do Crédito – M105” associados ao registro M100\. Observar que este recálculo é necessário porque os valores foram alterados na tela de “Detalhamento da Base de Cálculo do Crédito – M105” e esta coluna é o total informado nestes registros filhos\.
- Campo “Valor Total do Crédito Apurado no Período”: uma vez que o conteúdo do campo “Valor da Base de Cálculo do Crédito” foi atualizado, o valor demonstrado neste campo também é impactado, de modo que ele deve ser recalculado para refletir o resultado do cálculo: Valor da Base de Cálculo do Crédito __vezes__ \(Alíquota do PIS __dividido__ por 100\)\.
