# MTZ_EFD_Pre_Geracao_Ressarcimento_ICMSST_MG_Calculo_ICMS_ST_C430

- **Fonte:** MTZ_EFD_Pre_Geracao_Ressarcimento_ICMSST_MG_Calculo_ICMS_ST_C430.docx
...
Processamento do Cálculo ICMS\-ST – Registro C430
...
Alteração da regra de preenchimento do campo 06\-VL\_UNIT\_ICMS\_NA\_OPERACAO\_CONV\.  O cálculo do campo passará a considerar a alíquota do FCP, além da alíquota interna do produto\.

Andréa Rocha

# <a id="_Toc462320892"></a><a id="_Toc27038220"></a><a id="_Toc37749427"></a>INTRODUÇÃO* *

Este documento contém as regras de geração do registro C430\.
...
<a id="RP09_C430"></a>RP09

Quando o parâmetro __*C430\- Saída de mercadorias sujeitas à substituição *tributária \(02, 02D e 60\) __estiver marcado na tela: Todos os cálculos realizados por este processo serão armazenados na tabela x302\_inf\_comp\_st\_res\_it\_ecf, referente à saída e serão utilizados para a geração do registro C430 do SPED FISCAL\.

## 1 – Recuperação dos Cupons Fiscais 

__Critérios da recuperação dos Cupons Fiscais \(SAFX993 / SAFX994\): __ 

 

- Empresa – código da empresa do login; 
- Estabelecimento – código do estabelecimento selecionado para geração do cálculo; 
- Data Emissão – data enquadrada no período informado em tela \(Tela Cálculo ICMS\-ST\); 
- Produto ou NCM \(sujeito ao ICMS\-ST\)\*\* 
- Somente cupons não cancelados \(somente os cupons com o campo “10\-Situação do Documento” da SAFX993 = 1\);
- Somente cupons de modelo 02, 2D ou 60 \(campo “03\-Modelo Documento” da SAFX993\); 
- Somente cupons com o campo “09\-Tipo do Documento” da SAFX993 = 1 \(Cupom Fiscal\);  \(as opções “2” e “3” não se referem à movimentação de mercadoria\)
...
__\*\*\* Conversão de Unidade, para encontrar a Quantidade Convertida \(SAFX994\):__

__Se__ unidade da nota __\(\*\)__ = unidade do cadastro do produto __\(\*\*\) __

      __\(\*\)__ unidade da nota = campo “17\-Unidade de Medida” da SAFX994

      __\(\*\*\)__ unidade do produto = campo “14\-Código de Medida” da SAFX2013 

      Nesse caso não há necessidade de conversão, e o campo será gerado a própria Quantidade do Item \(SAFX994 – campo 16\);

__Senão __

Neste caso a quantidade do item será convertida para a unidade de medida do cadastro do produto, utilizando o fator de conversão obtido nas tabelas de Conversão de Medida\.

Assim, será a multiplicação dos seguintes campos:

          \[Quantidade do item \(SAFX994 – campo 16\) \* Fator de Conversão\]
...
\- Data = Data de Emissão do Cupom fiscal \(SAFX993\);
...
A partir da MFS\-67654, a regra para saber se houve complemento ou ressarcimento não é feita mais pela comparação das bases de entrada x saída\.  Sendo assim, a regra abaixo deixa de ser utilizada:

No item [__Restituição/Complemento__](#RP02_Restituicao_Complemento)__:__

__Considerar para a Restituição/Complemento as seguintes fórmulas:__

__Considerar para os Itens de saída, de acordo com a origem__ \(Realizar a conversão da unidade do item de saída, se necessário\):

SAFX994: VLR\_UNIT \(não há redução de base\)

__Comparar com o valor \(médio, unitário\) dos Itens de Entrada__

SAFX08: 61 \- BASE\_SUB\_TRIB\_ICMS

__Validações__

Considerar as validações na RP05 \([Mensagens que devem ser apresentadas no Log do processamento](#RP05_GERAL) e [Mensagens que devem ser apresentadas no Log do processamento \(C430\)\)](#RP05_GERAL_c430)

MFS\-35339

MFS\-67654

<a id="RP10_tabela_registro_C430"></a>RP10

Gravar os registros na tabela x302\_inf\_comp\_st\_res\_it\_ecf, conforme regras indicadas nos respectivos campos indicado na tabela abaixo__\.  Para os registros C430__, apresentar __APENAS__ os cupons fiscais da SAFX993/SAFX994 de __modelo igual a 02, 2D e 60__, que foram recuperados na [RP09](#RP09_C430)\)\.

Os dados calculados e gravados podem ser consultados na tela:  Menu Básicos, Módulo: DW, menu Manutenção à Cupom Fiscal \(EFD/REDF\) à Movimentos à Detalhe Informações Complementares Oper\. ST \(Sped Fiscal\) – Redução Z

__\(Tabela de Gravação dos Registros C430\)__
...
Considerar a informação do campo 01\-COD\_EMPRESA do documento de saída recuperado da SAFX994\.
...
Considerar a informação do campo 02\-COD\_ESTAB do documento de saída recuperado da SAFX994\.
...
Considerar a informação do campo 03\-COD\_MODELO do documento de saída recuperado da SAFX994\.
...
Considerar a informação do campo 04\-COD\_CAIXA\_ECF do documento de saída recuperado da SAFX994\.
...
Considerar a informação do campo 17 \- NUM\_CRZ do documento de saída recuperado da SAFX993\.
...
Considerar a informação do campo 06\-Data da Emissão do documento de saída recuperado da SAFX994\.
...
COD\_TOTALIZADOR\_ECF

Considerar a informação do campo 15\-COD\_TOTALIZADOR\_ECF do documento de saída recuperado da SAFX994\.
...
Considerar a informação do campo 08\-IND\_PRODUTO do documento de saída recuperado da SAFX994\.
...
Considerar a informação do campo 09\-COD\_PRODUTO do documento de saída recuperado da SAFX994\.
...
Se o valor ‘Valor unitário da saída da mercadoria’  for inferior  ao Valor Base ICMS\-ST Unitário Médio\) dos Itens de Entrada do dia’,__ __calculado conforme__ __[__PrecoMedioDIA__](#PrecoMedioDIA), teremos __restituição__\. O motivo será igual a __MG100__
...
Se \(16\-VLR\_UNIT\_ICMS\_ESTQ\_CONV \+ 

      17\-VLR\_UNIT\_ICMSS\_ESTQ\_CONV\) \- 

      14\- VLR\_UNIT\_ICMS\_OPER\_CONV > 0 
...
então:
    ...
    Nesse caso o cupom não será considerado na 
...
Ver regra de conversão indicada na indicada na RP09
...
Campo 14 – Unidade de Medida do Cadastro do Produto \(SAFX2013\), do produto pertencente ao item do cupom\.
...
Considerar o 18\-VLR\_UNIT do item do documento de saída recuperado da SAFX994\.
...
\[__MFS988342__\] Inclusão da alíquota do FCP somada à alíquota interna no cálculo do valor 

Considerar o resultado da expressão: valor do contábil do documento de saída \(22\-Valor Total Líquido, da SAFX994 multiplicado pela alíquota\)/quantidade do item de saída recuperado \(convertida\)

__Como identificar a Alíquota:__

Considerar a alíquota Interna \+ Alíquota FCP existentes na parametrização de Produto ou NCM sujeito ao ICMS\-ST\.

O somatório das alíquotas recuperadas deve ser divido por 100\.
...
Se campo 24 – COD\_MOTIVO\_SAI igual a MG100 ou MG300 deixar este campo vazio\.
...
Considerar o valor unitário médio calculado na RP02, \(__\*\*Valor ICMS UNITARIO Médio\)__,__ calculado conforme __[__PrecoMedioDIA__](#PrecoMedioDIA)__, __referente ao dia do cupom\. \(Operações de Entrada\)

Preencher com o “Valor Médio Unitário do ICMS” recuperado do “Cálculo da Média Pondera Móvel dos Valores Unitários” \(EST\_ST\_MG\_MEDIA\_POND\), conforme tópico 2 da RP09, para o Produto na Data de Emissão do cupom\. 
...
Considerar o valor unitário médio dos campos ICMS ST \+ VLR\_FECP\_ICMS\_ST calculado na RP02,  __conforme __[__PrecoMedioDIA__](#PrecoMedioDIA), referente ao dia do cupom\.\(Operações de Entrada\)

Preencher com “Valor Médio Unitário do ICMS\-ST c/ FECEP” recuperado do “Cálculo da Média Pondera Móvel dos Valores Unitários” \(EST\_ST\_MG\_MEDIA\_POND\), conforme tópico 2 da RP09, para o Produto na Data de Emissão do cupom\. 
...
Considerar o valor unitário médio \(__Valor FECP\-ST UNITARIO Médio__\) calculado na RP02, __conforme __[__PrecoMedioDIA__](#PrecoMedioDIA),, referente ao dia do cupom\. \(Operações de Entrada\)

Preencher com “Valor Médio Unitário do FECEP\-ST” recuperado do “Cálculo da Média Pondera Móvel dos Valores Unitários” \(EST\_MG\_RS\_MEDIA\_POND\), conforme tópico 2 da RP09, para o Produto na Data de Emissão do cupom\. 
...
Se o campo 10 – COD\_MOTIVO\_SAI for igual a MG100, preencher com o resultado da expressão: 

\(16\-VLR\_UNIT\_ICMS\_ESTQ\_CONV\) \+

\(17\-VLR\_UNIT\_ICMSS\_ESTQ\_CONV\) – 

\(14\- VLR\_UNIT\_ICMS\_OPER\_CONV\)
...
“O resultado do cálculo: ‘Valor Unitário ICMS Estoque Conv\.’  \+ ‘Valor Unitário ICMS\-ST Estoque Conv\.’ \- Valor Unitário ICMS na Oper\. Conv\.

 ficou negativo,  por isso, o campo ‘Valor Unitário ICMS\-ST Conv\. Rest\.’ foi gravado com zero”\.

Apresentar no log, as informações para conseguir identificar o item do cupom\.

Se o campo 10 – COD\_MOTIVO\_SAI igual a MG300, deixar este campo vazio\. 
...
Se o campo 10 – COD\_MOTIVO\_SAI for igual a MG100 e campo 18 \- VLR\_UNIT\_FCPS\_ESTQ\_CONV for maior que zero, preencher com o resultado da expressão: \(19\- VLR\_UNIT\_ICMSS\_REST\_CONV \* a alíquota de FCP\)\.
...
Se o campo 10 – COD\_MOTIVO\_SAI for igual a MG300, preencher com o resultado da expressão: 
...
Se o campo 10 – COD\_MOTIVO\_SAI igual a MG100, deixar este campo vazio\.
...
Se o campo 10 – COD\_MOTIVO\_SAI for igual a MG300 e campo 18 \- VLR\_UNIT\_FCPS\_ESTQ\_CONV for maior que zero, preencher com o resultado da expressão: \(campo 21\- VLR\_UNIT\_ICMSS\_COMPL\_CONV \* a alíquota de FCP\)\.
...
Todos os registros gravados na tabela x302\_inf\_comp\_st\_res\_it\_ecf, através deste processo [\(RP10\)](#RP10_tabela_registro_C430), devem ser identificados que foram calculados pelo sistema, pois os registros importados \(SAFX302\) não podem ser alterados \(por esse processo\)\.
