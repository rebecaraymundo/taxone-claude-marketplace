...
Este documento de regras é específico da geração do Bloco G desenvolvida para o cliente NEXTEL \(geração efetuada a partir do parâmetro “__*Gerar os registros a partir de valores calculados pelo Módulo Ativo Permanente no mês anterior à Apuração do ICMS*__” do menu “Dados Iniciais”\)\. 
...
\- Correção de duplicidades de registros;

\- Retirada do parâmetro “*Não demonstrar transferências entre estabelecimentos da mesma UF na geração por IE Única”;*

*\- Alteração na geração do NUM\_PARC do G126; *
...
Alteração na geração do campo 04 \- TIPO\_MOV do registro G125\.
...
Alterações do Guia Prático vrs 3\.0\.2  \- alteração dos registros G130 e G140\.
...
Inclusão do tratamento do novo parâmetro que vai indicar a forma de demonstrar os campos 6 e 7 do registro G110\.
...
__MFS84103\] __Inclusão de um parâmetro para indicar a forma de recuperar os campos 6 e 7 do Registro G110 para o período anterior

Os dados deste registro serão gerados a partir do Módulo CIAP, considerando os dados de movimento do CIAP do mês anterior ao de referência da geração do SPED \(ex\.: se foi indicado para geração do SPED o mês de fevereiro, consideraremos dados do CIAP de janeiro\)\.  Exceto os campos 6 \(VL\_TRIB\_EXP\) e 7 \(VL\_TOTAL\), que serão gerados com base nos movimento do CIAP do mês atual, quando o parâmetro “Gerar os campos 6 e 7 do Registro G110 a partir do período da Apuração do ICMS “ estiver marcado na tela de Dados Iniciais\.

	

Mas as datas inicial e final do período que serão mostradas, serão as mesmas do E100, porque se mostrarmos as datas referentes ao período anterior, dará erro no validador\.
...
As regras de geração dos registros do Bloco G dependem do modelo de livro utilizado \(C ou D\), e também do formato do livro\. A identificação destes dados é feita através da parametrização do CIAP, na tabela __APT\_ESTAB __\(menu “Cadastros 🡪 Parâmetros de Estabelecimento”\), da seguinte forma:
...
__OBS2 \(OS2931\-B12\)__: Na geração da opção “*Geração \- PIM*” o Bloco G só será gerado para o arquivo da inscrição estadual parametrizada no Módulo CIAP \(campo “Inscrição Estadual p/AM” do item “Cadastros 🡪 Parâmetros por Estabelecimento”\)\. Os dados do Bloco G serão gerados a partir da apuração do CIAP feita para o Estabelecimento\. Nos arquivos das demais inscrições estaduais o Bloco G será gerado sem informação\.  
...
Se o parâmetro “Gerar os campos 6 e 7 do Registro G110 a partir do período da Apuração do ICMS” da tela de Dados Iniciais estiver desmarcado

     Preencher com o valor das saídas tributadas \+ exportações  do __período anterior__, que 

     correspondente ao campo descrito a seguir, dependendo do modelo do livro do CIAP

Senão

     Preencher com o valor das saídas tributadas \+ exportações  do __período atual__, que 
...
Se Modelo = “C”
      ...
      \(corresponde ao campo “Total das Op\. de Saída Tributadas” do menu “Movimento 🡪 Manutenção

        Dados do Cálculo 🡪 Modelo C 🡪 Cálculo Geral dos Créditos”\)  

Se Modelo = “D” 

      Preencher com o conteúdo da coluna VLR\_TOT\_OP\_ISENTAS da tabela APT\_EST\_MENS\_ANO

     \(corresponde ao campo “Total de Operações Tributadas” do menu “Movimento 🡪 Manutenção

       Dados do Cálculo 🡪 Modelo D 🡪 Fator de Cálculo”\) 
...
Se o parâmetro “Gerar os campos 6 e 7 do Registro G110 a partir do período da Apuração do ICMS” da tela de Dados Iniciais estiver desmarcado
    ...
    Preencher com o valor total das saídas do __período anterior__, que correspondente ao campo descrito 

    a seguir, dependendo do modelo do livro do CIAP

Senão

    Preencher com o valor total das saídas do __período atual__, que correspondente ao campo descrito a 

    seguir, dependendo do modelo do livro do CIAP

Se Modelo = “C”

      Preencher com o conteúdo da coluna VLR\_TOT\_OP\_SAIDAS da tabela APT\_DEM\_CR\_MENSAL

      \(corresponde ao campo “Total Geral das Op\. de Saída” do menu “Movimento 🡪 Manutenção
...
Se Modelo = “D” 
     ...
     \(corresponde ao campo “Total das Operações de Saída” do menu “Movimento 🡪 Manutenção
...
__Alteração da MFS14642: __O registro G125 será gerado sem destaque de valor a ser apropriado no período, ou seja, os campos NUM\_PARC e VL\_PARC\_PASS serão gravados sem valor\.  O registro G126 será gerado com o movimento do período de seleção dos dados do CIAP, que deveria sair no G125\.

Neste registro serão gravadas todas as movimentações dos Bens que compõe a apuração do CIAP no período anterior ao informado para a geração\. 
...
Se Modelo = “C”🡪 utilizar a tabela APT\_DEM\_BASE\_CR

Se Modelo = “D”🡪 utilizar as tabelas APT\_EST\_SAIDA  e  APT\_ALIENACAO

No caso do __Modelo C__  os movimentos ficam em uma única tabela, a APT\_DEM\_BASE\_CR:

     Movimentos de entrada 🡪 linhas da APT\_DEM\_BASE\_CR com IND\_E\_S = ‘E’

     Movimentos de saída   🡪  linhas da APT\_DEM\_BASE\_CR com IND\_E\_S = ‘S’
...
No caso do __Modelo D__  são utilizadas duas tabelas, uma para as entradas e outra para as saídas:
     ...
     Movimentos de entrada 🡪  APT\_EST\_SAIDA

     Movimentos de saída    🡪  APT\_ALIENACAO

    

Critérios para recuperação dos dados:

APT\_DEM\_BASE\_CR 🡪 COD\_EMPRESA, COD\_ESTAB e DAT\_APURACAO = data final do mês anterior

APT\_EST\_SAIDA    🡪 COD\_EMPRESA, COD\_ESTAB e DAT\_APURACAO = data final do mês anterior

APT\_ALIENACAO  🡪 COD\_EMPRESA, COD\_ESTAB e DAT\_OPER >= DT\_INI e <= data final do mês anterior
...
🡺Originalmente definimos a geração para gravar as baixas por término do período de apropriação somente no mês seguinte ao da última parcela, seguindo as regras do GP vrs 2\.0\.0\. \(registro G125, campo 4, item 3\), que coincide com as regras do livro Modelo C do CIAP\. No entanto, no GP vrs 2\.0\.1, a regra foi alterada para gravar estas baixas no mês da ocorrência \(registro G125, campo 4, item 5\)\. 
...
- Data Fiscal = deve estar compreendida no período anterior ao período informado para a geração 
- Status do CIAP = “I” \(Informativo\)

Para os Componentes baixados no período \(alteração OS2388\-E22\):

- Data da alienação deve estar compreendida no período anterior ao período informado para a geração
- Status do CIAP = “I” \(Informativo\)
...
Sobre o campo DT\_MOV do G125 🡺 Quando duas aquisições a serem totalizadas tiverem datas de operação diferentes, a data a ser gravada no G125 será ser a *menor data* \(exemplo: aquisições do Bem __B700__ do cenário de teste\)\.
...
Sobre o tipo do movimento do G125 🡺 O tipo do movimento do G125 também deveria constar dos critérios para efetuar a totalização, pois é campo chave deste registro\. No entanto, observa\-se que no caso das entradas, o tipo de movimento será sempre o mesmo, independente do número de aquisições\. Isso porque nas entradas o campo 04\-TIPO\_MOV é gerado a partir do indicador da SAFX13 \(campo “Tipo do Bem”\), e como todas as aquisições estão relacionadas ao mesmo Bem, o tipo de movimento será sempre o mesmo\.
...
Baixas do tipo “Parcial” 🡪 Estas baixas servem apenas para deduzir os valores de ICMS a serem demonstrados no movimento de entrada, pois elas *não* são gravadas no arquivo\. Neste caso, dependendo de como a totalização for realizada, temos duas situações:

Baixas do tipo “Total” 🡪 Fazer um grupamento dos registros *para evitar a repetição de registros iguais*, lembrando que no caso das saídas o registro G125 é gravado sempre *sem* informação nos campos de valor \(campo 05 ao 10\)\. Logo, *não existem valores a serem totalizados*, basta evitar a gravação de baixas de mesmo Código de Bem e Tipo de Movimento;

Sobre o campo DT\_MOV do G125 🡺 A data do movimento \(campo DT\_MOV do G125\) *não* deve ser considerada no grupamento, e quando duas baixas a serem grupadas tiverem datas diferentes, a data a ser gravada no G125 será ser a *menor data* \(exemplo: baixas do Bem __B005__ do cenário de teste\)\.

OBS 🡪 A chave do registro G125 é COD\_IND\_BEM  \+  TIPO\_MOV \(até o Guia Prático vrs 2\.0\.2\)
...
__Regra geral sobre Geração por Inscrição Estadual Única__:     __\(MFS22137\)__

As regras a seguir foram definidas para eliminar problemas de duplicidade de registros em caso de geração por inscrição estadual única\. Nestes casos, quando ocorre a transferência de Bens entre estabelecimentos envolvidos na centralização, a presença de Bens de mesmo código em estabelecimentos diferentes ocasiona duplicidade do registro G125 *\(no documento de requisitos foi descrito exemplo do problema\)*\. Para solucionar o problema, a geração do G125, tanto de entrada, como de saía, seguirá a regra abaixo:

*\(lembrando que a regra é específica para geração por inscrição estadual única\)*

__Movimento de entrada do Bem: __

\- Será gerado um único registro G125 de entrada, sempre considerando o registro da aquisição 

  \(APT\_AQUISICAO\) mais antigo, ou seja, a aquisição do Bem no estabelecimento de origem\.

  Para obter o registro da aquisição original, basta considerar a menor data de operação do CIAP

  \(DAT\_OPER\);

*  *

\- As regras da geração dos campos DT\_MOV e TIPO\_MOV do G125 não se alteram, mas, cabe

  ressaltar que as regras da geração destes dois campos, utilizam a informação da data de operação do

  Bem \(DAT\_OPER\) para verificar o conteúdo a ser gerado nos dois campos\. E nesse caso, será

  utilizada como parâmetro a data de operação do registro da aquisição utilizado na geração do G125,

  conforme citado acima;

\- Consequentemente, os registros “filhos” G130 e G140 serão gerados com base no registro da

  aquisição utilizado na geração do G125;

  

__Movimento de saída do Bem:__

\- Será gerado um único registro G125 de saída, sempre considerando o registro da alienação

  \(APT\_ALIENACAO\) mais recente, ou seja, o último movimento de saída que exista para o Bem no

  período, considerando a regra de recuperação dos movimentos de saída \(lembrando que conforme a

  regra, as baixas parciais *não* são consideradas para a geração do G125; 

\- Consequentemente, os registros “filhos” G130 e G140 serão gerados com base no registro da

  saída utilizado na geração do G125;

Este procedimento tem por objetivo eliminar duplicidades, como por exemplo, quando existir várias transferências de um Bem no mesmo período, entre os estabelecimentos da I\.E\.U\. Ou, quando existir transferência e baixa do Bem por término do período de apropriação\. 

= = = = = fim __Regra geral sobre Geração por Inscrição Estadual Única__ __\(MFS22137\)__
...
A consulta destes créditos é feita na “Tabela dos Créditos Extemporâneos \(Integral\)”\. Esta tabela é gerada no seguinte menu: “*Créditos Extemporâneos \(Integral\) 🡪 Cálculo dos Créditos*”
...
Obs 🡪 O incorporador é opcional, e quando não existe, o campo no Mastersaf é gravado com  um caracter em branco \(pois é chave da X13\)\. Logo, quando não houver incorporador, o campo deverá ser preenchido apenas com o código do Bem, *sem fazer a concatenação*\.
...
No caso desta geração específica da Nextel, os dados do CIAP são recuperados do mês anterior ao período da geração da EFD, e neste caso, nunca existirão entradas ou saídas no período\.
...
Nos movimentos de entrada:
    ...
    \(no campo 04\-TIPO\_MOV, será gravado sempre “SI”, exceto quando a primeira entrada do bem \(primeiro crédito\) ocorrer no registro G126, que será preenchido sempre com “__IM__”\.__\[MFS25185\]__\)

Nos movimentos de saída:

    Para o modelo C 🡪 será a DAT\_OPER da APT\_DEM\_BASE\_CR

    Para o modelo D 🡪 será a DAT\_OPER da APT\_ALIENACAO

Observar que na geração por I\.E\.U\., quando existir mais de um saída para o Bem no perído, será considerado apenas *o movimento da saída mais recente*\. Este cenário pode ocorrer quando existem transferências do Bem entre os estabelecimentos envolvidos na centralização \(ver regras específicas da geração por I\.E\.U\. na __RNG125__, alteração da__ MFS22127__\)\.

RNG125\-04

Campo TIPO\_MOV:

__Nos movimentos de entrada \[MFS25185\]__:

No caso das entradas este campo será preenchido sempre com “__SI__”, exceto quando a primeira entrada do bem \(primeiro crédito\) ocorrer no registro G126, que será preenchido sempre com “__IM__”\.

No caso desta geração específica da Nextel, os dados do CIAP são recuperados do mês anterior ao período da geração da EFD, e neste caso, os Bens sempre terão data da entrada no CIAP em períodos anteriores \(a DAT\_OPER da aquisição será sempre de período anterior\)\. Portanto, serão sempre “SI”, quando não for a primeira entrada do bem \(primeiro crédito\) no registro G126\. 
...
__Alteração da MFS14642: __Quando o parâmetro “Não demonstrar transferências entre estabelecimentos da mesma UF na geração por IE Única” da tela de Dados Iniciais \(Parâmetros 🡪 Dados Iniciais\) for selecionado e a opção de Geração por Inscrição Estadual Única na geração do Meio Magnético for selecionada, os movimentos de saída por transferência \(TIPO\_MOV = AT\) não devem ser demonstrados, se o estabelecimento de destino possuir a mesma UF do estabelecimento de origem\.

*\(este parâmetro foi retirado da tela na MFS22137\)*
...
Obs\.: O número da parcela será gerado no registro G126, como parcela extemporânea\.
...
Obs\.: O valor passível de apropriação será gerado no registro G126, como valor extemporâneo\.
...
__No caso de Créditos Extemporâneos \(Bens originados da APT\_AQUISICAO\):__
...
Data de Apuração      = data final do mês anterior ao mês informado para a geração
...
__No caso de Créditos “normais” \(Bens originados da APT\_AQUISICAO\):__

*\(créditos do CIAP que seriam normalmente gerados no registro G125\) *

Origem dos dados: Os movimentos correspondem aos dados demonstrados nos livros do CIAP, que são as informações sobre a NF da entrada do Bem\.
...
Serão seguidas as mesmas regras de geração do registro G125, pois os valores do G125 \(NUM\_PARC e VL\_PARC\_PASS\) serão gravados no registro G126\. 
...
A geração deste campo depende da origem do crédito, ou seja, se é um crédito extemporâneo de fato, ou, se é um crédito do CIAP “normal” que deveria ir para o G125, mas que nesta geração específica vai para o G126

__Quando se tratar de Crédito Extemporâneo__: \(Tabela dos Créditos Extemporâneos\) 

  

__     Se__ o campo “*Número da Parcela Sequencial*” estiver preenchido,

           Nesse caso o campo NUM\_PARC será preenchido com o conteúdo do campo “*Número da*

*           Parcela Sequencial*”;

__     Senão__

          Nesse caso o campo NUM\_PARC será preenchido com o conteúdo do campo “*Parcela*”;

__Quando se tratar de Crédito “normal”__: \(Tabelas dos Cálculos do CIAP, Mod C ou D\)

*\(créditos do CIAP que seriam normalmente gerados no registro G125\) *

     __Se__ o campo “*Número da Parcela Sequencial*” estiver preenchido,
...
__     Senão__
          ...
          Nesse caso, o campo NUM\_PARC será preenchido com o número da parcela que será

          calculado, utilizando a mesma regra de cálculo do número da parcela descrita no documento de

          regras “Sped\_Fiscal\_Regras\_Bloco\_G”, na  __RNG125\-09 __\(\*\*\* OBS \*\*\*\);

*\*\*\* OBS: \*\*\* Sobre o cálculo do número da parcela, a diferença nesta geração específica, é apenas em relação ao parâmetro “Data de Apuração” citado na regra\. Como nesse caso estamos trabalhando com valores referentes ao mês anterior, a data de apuração a ser usada no cálculo da parcela deve ser a data final do mês anterior \(caso contrário seria calculada a parcela referente ao período da geração\)\.*

*\(A coluna “Número da Parcela Sequencial*”* foi criada nas tabelas dos créditos do CIAP na *__*MFS22137*__*\)*

RNG126\-05

Campo VL\_PARC\_PASS:

__Crédito Extemporâneo:__

Este campo é preenchido com o valor da parcela passível de apropriação calculada para o Crédito Extemporâneo\.

__Alteração da MFS14642:__
...
Se  \(Modelo = “C”  e  VLR\_CRED\_MENSAL da APT\_DEM\_BASE\_CR <> zeros\)  ou

       \(Modelo = “D” e VLR\_TOT\_EST\_MENSAL da APT\_EST\_SAIDA <> zeros\)

Se  \(Modelo – “C”  e  VLR\_PASS\_APROP \(\*\) da APT\_DEM\_BASE\_CR <> zeros\) ou 
...
\(\*\) OS2931\-B13 / CH31587: Alterada a geração deste campo, que passou a utilizar a informação da nova coluna criada nas tabelas de cálculo do CIAP \(“Valor Passível de Apropriação”\) p/armazenar o valor do crédito passível de apropriação \(valor da parcela antes de aplicar o coeficiente das saídas\)\.

Obs: No caso dos bens do tipo “componente” sem direito a crédito, o campo não será preenchido \(‘||’\), pois estes bens não entram no cálculo do CIAP\.  São os Bens com status = “I” que são recuperados direto da APT\_AQUISICAO, pois não se encontram nas tabelas do cálculo \(ver detalhes na RNG125\)\.
...
Este campo é preenchido com o Valor do somatório das saídas tributadas e saídas para exportação no período calculado para o Crédito Extemporâneo\.

__Alteração da MFS14642:__

Este campo é preenchido com o Valor do somatório das saídas tributadas e saídas para exportação \(VL\_TRIB\_EXP\) do registro G110\.
...
Este campo é preenchido com o Valor total de saídas no período calculado para o Crédito Extemporâneo\.

__Alteração da MFS14642:__

Este campo é preenchido com o Valor total de saídas \(VL\_TOTAL\) do registro G110\.
...
Campo VL\_PARC\_APROP

__Crédito Extemporâneo:__

Este campo é preenchido com o Valor de outros créditos de ICMS a ser apropriado na Apuração, calculado para o Crédito Extemporâneo\.

__Alteração da MFS14642:__

Este valor representa o Valor de outros créditos de ICMS a ser apropriado na apuração, e deve ser preenchido com o seguinte conteúdo: 

                        \( campo 05\- VL\_PARC\_PASS  \*  campo 08\- IND\_PER\_SAI\)
...
Nos casos em que o G125 for um movimento de entrada tipo “CI” \(entrada de Bens Resultantes\), os registros G130 e G140 *não devem ser gerados* serão gerados com base nas NFs registradas nos componentes associados ao bem resultante gerado\.
...
__Se Demais Tipos: __
     ...
     Se __entrada__ 🡪 os dados devem ser recuperados do cadastro das aquisições 

                             \(__APT\_AQUISICAO__\)

     Se __saída__ 🡪 os dados devem ser recuperados do cadastro das alienações
...
Se __entrada__ 🡪 os dados devem ser recuperados do cadastro das aquisições \(__APT\_AQUISICAO__\)

Se __saída__ 🡪 os dados devem ser recuperados do cadastro das alienações \(__APT\_ALIENACAO__\)
...
Alteração __MFS22137__: Quando se tratar de geração por Inscrição Estadual Única __e__ o parâmetro “*Gerar a numeração das parcelas de forma sequencial nos casos de transferência \(Inscrição Estadual Única\)*” estiver marcado, será utilizada a DAT\_OPER do registro do Bem de menor data \(menor DAT\_OPER\) que existir na tabela das aquisições, considerando todos os estabelecimentos envolvidos na centralização\. Este procedimento garantirá que em caso de Inscrição Estadual Única, sejam consideradas as parcelas já calculadas anteriormente em outros estabelecimentos \(ver regras específicas da geração por I\.E\.U\. na __RNG125__, alteração da__ MFS22127__\)\.   

🡺 Se periodicidade for mensal  \(IND\_PERIODIC\_102  = ‘M’\):

Calcular o número de meses existente entre DAT\_OPER e Data da Apuração, incluindo o último mês\.
...
🡺 Se periodicidade for quinzenal  \(IND\_PERIODIC\_102  = ‘Q’\):
...
Passo 2 🡪 calcular o número de meses existente entre DAT\_OPER e Data da Apuração, da mesma forma feita para a periodicidade mensal\. 
...
🡺 Se periodicidade for decendial  \(IND\_PERIODIC\_102  = ‘D’\):
...
Passo 2 🡪 calcular o número de meses existente entre DAT\_OPER e Data de Apuração,  da mesma forma feita para a periodicidade mensal\. 
...
__ETAPA II – Verificar a existência de parcelas extemporâneas:__
...
__ETAPA III – Calcular o total de parcelas calculadas:__
