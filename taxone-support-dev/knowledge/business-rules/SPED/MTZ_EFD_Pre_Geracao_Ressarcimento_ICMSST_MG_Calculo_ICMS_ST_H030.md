# MTZ_EFD_Pre_Geracao_Ressarcimento_ICMSST_MG_Calculo_ICMS_ST_H030

> Fonte: MTZ_EFD_Pre_Geracao_Ressarcimento_ICMSST_MG_Calculo_ICMS_ST_H030.docx
...
Processamento do Cálculo ICMS-ST – Registro H030
...
Este documento contém as regras de geração do registro H030.
...
| --- | --- | --- |
| RP15 | Quando o parâmetro ‘H030 - Inventário das mercadorias sujeitas ao regime de substituição tributária’,  e a opção ‘Sobrepor os valores calculados na tabela de inventário (Registro H030)’, estiverem selecionados na tela: Todos registros de inventário recuperados, serão atualizados com os cálculos realizados por este processo (tabela x52_invent_produto), ou seja, se existir antes do processo de cálculo, valores nos campos, os mesmos serão sobrepostos.  Se a opção ‘Sobrepor os valores calculados na tabela de inventário (Registro H030)’ estiver desmarcado, apenas os registros que não possuem os campos (21-VLR_ICMS_MEDIO, 22-VLR_ICMSS_MEDIO, 43-VLR_BASE_ICMSS_MEDIO e 44-VLR_FCP_MEDIO) preenchidos serão atualizados, ou seja, se pelo menos um desses campos maior que zero, o registro não será considerado no cálculo. ‘ As recuperação dos produtos e a regra do cálculo do preço médio está descrita na  RP00A  “MTZ_EFD_Pre_Geracao_Ressarcimento_ICMSST_MG_Calculo_ICMS_ST.docx”. Origem: tabela EST_ST_MG_SALDO_MED Preencher os campos da X52 conforme regra a seguir:  21-VLR_ICMS_MEDIO:  Valor Médio Unitário do ICMS calculado na RP00A  (campo  vlr_med_unit_icms da  EST_ST_MG_SALDO_MED) [MFS-58456] 22-VLR_ICMSS_MEDIO:  Valor Médio Unitário do ICMS-ST Valor Médio Unitário do ICMS-ST c/ FECEP calculado na RP00A  (campos vlr_med_unit_icmss e vlr_med_unit_fecp da EST_ST_MG_SALDO_MED) 43-VLR_BASE_ICMSS_MEDIO:  Valor Médio Unitário da Base ICMS-ST calculado na RP00A (vlr_med_unit_bc_icmss da  EST_ST_MG_SALDO_MED) 44-VLR_FCP_MEDIO: Valor Médio Unitário FECEP-ST calculado na RP00A (campo vlr_med_unit_fecp da EST_ST_MG_SALDO_MED)  Validações  Considerar as validações na RP05 (Mensagens que devem ser apresentadas no Log do processamento e Mensagens que devem ser apresentadas no Log do processamento (H030) | MFS-36146 |
| RP16 | Gravar as informações dos campos médios calculados na tabela x52_invent_produto, conforme regras indicadas na RP15.  Os campos que serão atualizados são: 21-VLR_ICMS_MEDIO, 22-VLR_ICMSS_MEDIO, 43-VLR_BASE_ICMSS_MEDIO e 44-VLR_FCP_MEDIO da tabela de inventário.  Os dados calculados e gravados podem ser consultados na tela:  Menu Básicos, Módulo: DW, item Manutenção à Estoque à Inventário por Produto. | MFS-36146 |
| RP17 | Cada registro que foi alterado pelo processo de cálculo da tabela de inventário, devem ser identificados que foram atualizados pelo sistema. Para atender este cenário, criar novo domínio no campo ind_gravacao, conforme sugerido abaixo opção 9:  Domínio completo do campo ind_gravacao:  Tipo/tamanho: CHAR(1)  Valores padrão do campo  1.  Incluído por Importação 2.  Atualizado por Importação 3.  Incluído por Importação / Atualizado por Digitação 4.  Incluído por Digitação 5.  Incluído por Digitação / Atualizado por Digitação 6.  Gerado pelo Sistema 7.  Gerado pelo Sistema  / Atualizado por Digitação 8. Gerado pelo Sistema  / Atualizado por Importação 9. Atualizado pelo Sistema | MFS-36146 |
